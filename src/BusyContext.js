...
const BusyBuyProvider = ({ children }) => {
    ...
    const handleOrderItem = async (CartItems) => {
        try {
            ...
            const docRef = await addDoc(collection(db, "UserOrders"), {
                userId: userId,
                Order: orderItems,
                Created_ts: new Date().toLocaleDateString()
            });
    ...
    const getMyOrder = async () => {
        ...
        try {
            const q = query(collection(db, "UserOrders"), where("userId", "==", userId));
            const querySnapshot = await getDocs(q);

            if (!querySnapshot.empty) {
                const orders = querySnapshot.docs.map(doc => doc.data());

                // Only update myOrder if the data has changed
                if (!isEqual(orders, myOrder)) {
                    setMyOrder(orders);
                }
    ...
    return (
        ...
        <BusyContext.Provider value={{ allProducts, userId, loading, myCarts, myOrder, getCart, getMyOrder, getProduct, handleSignUp, handleSignIn, handleForgotPassword, handleLogout, handleGoogleSignIn, handleAddCart, handleError, handleRemoveCart, handleIncQuantity, handleDecQuantity, handleOrderItem }}>
    {loading ? (
